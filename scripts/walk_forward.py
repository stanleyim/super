...
LOCKED: expanding, annual folds, h=20, fixed thresholds, frozen state_id
"""
...
OUT_DIR = os.path.join(ROOT, "data", "processed", "backtest", "walk_forward")
...
def fold_metrics(trades, port, year):
    if port is None or trades is None or len(trades)==0:
        return {"fold":year, "trades":0, "passed":False, "note":"no trades"}
    dates, pr, eq, nact = port
    pra = pr[nact>0]
    if len(pra)<2: return {"fold":year, "trades":len(trades), "passed":False, "note":"insufficient"}
    ...
    return {"fold":year, "trades":int(len(trades)),
            "sharpe_net":round(float(sh),3), "mdd":round(float(mdd),4),
            "cagr":round(float(cagr),4), "calmar":round(float(calmar),3),
...
def main():
    ...
    for ty in TEST_YEARS:
        ...
        if not tradeable:
            fold_results.append({"fold":ty, "trades":0, "passed":False, "note":"no tradeable"}); continue
        test = df[(df["date"]>=start) & (df["date"]<=end)]
        if len(test)==0:
            fold_results.append({"fold":ty, "trades":0, "passed":False, "note":"no test"}); continue
        trades = gen_trades(test, tradeable)
        if trades is None:
            fold_results.append({"fold":ty, "trades":0, "passed":False, "note":"no trades"}); continue
        trades["fold"] = ty; all_trades.append(trades)
        port = portfolio(trades, df, start, end)
        m = fold_metrics(trades, port, ty)
        fold_results.append(m)
        print(f"  {'✅' if m.get('passed') else '❌'} sh={m.get('sharpe_net','-')} mdd={m.get('mdd','-')}")
    ...
    print("\n" + "="*80)
    print(f"{'fold':>6}|{'trades':>7}|{'sharpe':>8}|{'mdd':>8}|{'cagr':>8}|{'win':>6}|{'pass':>5}")
    for r in fold_results:
        if "sharpe_net" in r:
            print(f"{r['fold']:>6}|{r['trades']:>7}|{r['sharpe_net']:>8.3f}|{r['mdd']:>8.4f}|"
                  f"{r['cagr']:>8.4f}|{r['win_rate_net']:>6.4f}|{'✅' if r['passed'] else '❌':>4}")
    print(f"\n  passed: {n_pass}/{len(fold_results)} | mean: {mean_s:.3f} | cv: {cv:.3f}")
